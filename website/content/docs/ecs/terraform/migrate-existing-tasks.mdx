...
- You should remove the `execution_role_arn` and `task_role_arn` fields. The `mesh-task` module creates the task and execution roles, by default. If you need to use existing IAM role(s), use the `task_role` and `execution_role` to pass in existing roles.
